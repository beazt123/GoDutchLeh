...
public class Bill {
  ...
  private void calculateSplit() {
    // call all items in this bill, calculate itemExpensesMap for each of them and aggregate into
    // mExpensesMap
    ...
    for (Item item : mItemsList.keySet()) {
      ...
      for (Member member : itemExpensesMap.keySet()) {
        ...
        if (mExpensesMap.containsKey(member)) {
          mExpensesMap.put(
              member,
              mExpensesMap.get(member) + itemValue); // update each member's expense for each item
        }
      }
    }
  }
